...
function doRectanglesOverlap(rect1, rect2) {
  ...
  || (rect2.top < rect1.width && rect2.left <= rect1.height)) return true;
  return false;
}
...
function findFirstSingleChar(str) {
  ...
  //   for (let j = str.length - 1; j > i; j -= 1) {
  //     if (str[i] !== str[j]) {
  //       return str[i];
  //     }
  //   }
...
function isCreditCardNumber(ccn) {
  const creditCard = String(ccn);
  let sum = 0;
  for (let i = 0; i < creditCard.length; i += 1) {
    let num = Number(creditCard[i]);
...
function getDigitalRoot(num) {
  let number = num;
  while (number > 9) {
    number = number.toString().split('').map((i) => +i).reduce((sum, i) => sum + i);
  }
  return number;
}
